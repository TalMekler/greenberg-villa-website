...
For each legal page (`/:lang/privacy`, `/:lang/terms`, `/:lang/accessibility`) in each language, opened with a *different* stored language: axe; `lang`/`dir` taken from the URL; one `h1` and no skipped levels; the document title names the page; skip link to `<main>`; a visible ring on every Tab stop through to the footer's Cookie settings button; no horizontal scroll at 320px. On the privacy page, the language picker must open the same page in the other language with focus kept on the pressed button; on the terms page, Cookie settings must open the cookie dialog with focus inside, and Esc must close it and return focus to the button.

For the cookie banner, on a first visit in each language: it shows three buttons; Reject and Accept have the same colour, size and weight; it reads right-to-left in Hebrew; no cookie, stored choice or third-party script exists before a choice; axe on the page and on the open dialog; Tab order is skip link, the banner's policy link, then Reject, Accept, Customize; Tab never leaves the open dialog; Esc closes it without choosing and returns focus to Customize; Reject stores a refusal, removes the banner, moves focus to `<main>` and announces the change; the choice survives a reload; and a choice older than 6 months is asked for again.
